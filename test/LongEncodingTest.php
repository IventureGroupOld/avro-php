...
class LongEncodingTest extends PHPUnit\Framework\TestCase
{
     ...
     * @return bool
     */
    public static function is_64_bit()
    {
        return (PHP_INT_SIZE == 8);
    }

    public function skip_64_bit_test_on_32_bit()
    {
        if (!self::is_64_bit())
            $this->markTestSkipped('Requires 64-bit platform');
    }

    public function skip_if_no_gmp()
    {
     ...
     * @param $actual_binary
     */
    public function assert_bit_shift($expected, $actual, $shift_type,
     ...
     * @param $rbin
     */
    public function test_bit_shift($val, $shift, $expected_lval, $expected_rval, $lbin, $rbin)
    {

        $this->skip_64_bit_test_on_32_bit();

        $lval = (int)((int)$val << $shift);
        $this->assert_bit_shift($expected_lval, (string)$lval,
            'lshift', $lbin, decbin($lval));
        $rval = ((int)$val >> $shift);
        $this->assert_bit_shift($expected_rval, (string)$rval,
     ...
     * @param $rbin
     */
    public function test_left_shift_gmp($val, $shift,
     ...
     * @param $rbin
     */
    public function test_right_shift_gmp($val, $shift, $expected_lval, $expected_rval,
                                         $lbin, $rbin)
    {
        $this->skip_if_no_gmp();
     ...
     */
    public function test_encode_long($val, $expected_bytes)
    {
        $this->skip_64_bit_test_on_32_bit();
     ...
     * @param $expected_bytes
     */
    public function test_gmp_encode_long($val, $expected_bytes)
    {
        $this->skip_if_no_gmp();
     ...
     * @param $bytes
     */
    public function test_decode_long_from_array($expected_val, $bytes)
    {
        $this->skip_64_bit_test_on_32_bit();
     ...
     * @param $bytes
     */
    public function test_gmp_decode_long_from_array($expected_val, $bytes)
    {
        $this->skip_if_no_gmp();
        $ary = array_map('ord', str_split($bytes));
     ...
     * @return array
     */
    public function bit_shift_provider()
    {
        // val shift lval rval
        return array(
            array('0', 0, '0', '0',
